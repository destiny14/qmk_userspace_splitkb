// Copyright 2024 splitkb.com (support@splitkb.com)
// SPDX-License-Identifier: GPL-2.0-or-later

#include QMK_KEYBOARD_H

enum layers {
    _WIN = 0,
    _MAC,
    _NAV,
    _SYM,
    _SYMMAC,
    _ADJUST,
    _NAVMAC
};

enum {
    TD_F12_SFTF21,
};

// Aliases for readability
#define WIN      DF(_WIN)
#define MAC      DF(_MAC)

#define SYM      MO(_SYM)
#define NAV      MO(_NAV)
#define SYMMAC   MO(_SYMMAC)
#define ADJUST   MO(_ADJUST)
#define NAVMAC   MO(_NAVMAC)

#define CTL_ESC  MT(MOD_LCTL, KC_ESC)
#define CTL_QUOT MT(MOD_RCTL, KC_QUOTE)
#define CTL_MINS MT(MOD_RCTL, KC_MINUS)
#define ALT_ENT  MT(MOD_LALT, KC_ENT)

// homerow mods
#define LGUI_A   LGUI_T(KC_A)
#define LCTL_A   LCTL_T(KC_A) // for macos
#define LGALT_S  LALT_T(KC_S)
#define LSFT_D   LSFT_T(KC_D)
#define LCTL_F   LCTL_T(KC_F)
#define LGUI_F   LGUI_T(KC_F) // for macos

#define RGUI_OE  RGUI_T(KC_SCLN)
#define RCTL_OE  RCTL_T(KC_SCLN) // for macos
#define RALT_L   RALT_T(KC_L)
#define RSFT_K   RSFT_T(KC_K)
#define RCTL_J   RCTL_T(KC_J)
#define RGUI_J   RGUI_T(KC_J) // for macos

// german symbols windows
#define SYM_DEG  LSFT(KC_GRV) // °
#define SYM_USCR LSFT(KC_SLASH) // _
#define SYM_LSQB RALT(KC_8) // [
#define SYM_RSQB RALT(KC_9) // ]
#define SYM_DACH KC_GRV // ^
#define SYM_BSLS RALT(KC_MINUS) // BACKSLASH
#define SYM_SLSH LSFT(KC_7) // /
#define SYM_LCUB RALT(KC_7) // {
#define SYM_RCUB RALT(KC_0) // }
#define SYM_STAR LSFT(KC_RBRC) // *
#define SYM_HASH KC_BSLS // #
#define SYM_DOLR LSFT(KC_4) // $
#define SYM_PIPE RALT(KC_NONUS_BACKSLASH) // |
#define SYM_MPIP LALT(KC_GRV) // |
#define SYM_TLDE RALT(KC_RBRC) // ~
#define SYM_BTCK LSFT(KC_EQL) // `
#define SYM_EXCL LSFT(KC_1) // !
#define SYM_LPOB KC_NONUS_BACKSLASH // <
#define SYM_RPOB LSFT(KC_NONUS_BACKSLASH) // >
#define SYM_EQL  LSFT(KC_0) // =
#define SYM_AMP  LSFT(KC_6) // &
#define SYM_QSTN LSFT(KC_MINUS) // ?
#define SYM_LBRK LSFT(KC_8) // (
#define SYM_RBRK LSFT(KC_9) // )
#define SYM_MINS KC_SLASH // -
#define SYM_COLN LSFT(KC_DOT) // :
#define SYM_PLUS KC_RBRC // +
#define SYM_PERC LSFT(KC_5) // %
#define SYM_DQOT LSFT(KC_2) // "
#define SYM_SQOT LSFT(KC_BSLS) // '
#define SYM_SCLN LSFT(KC_COMMA) // ;

// key combinations
#define ALT_INS  LALT(KC_INS) // Alt + Ins (Rider)
#define SFT_F12  LSFT(KC_F12) // Shift + F12 (Rider)
// tap dances
#define TD_F12S  TD(TD_F12_SFTF21)

tap_dance_action_t tap_dance_actions[] = {
    [TD_F12_SFTF21] = ACTION_TAP_DANCE_DOUBLE(LCTL(KC_F12), KC_F12),
};

bool isMac = false;

// clang-format off
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
/*
 * Base Layer: WIN
 *
 * ,-------------------------------------------.                              ,-------------------------------------------.
 * |  Esc   |   1  |   2  |   3  |   4  |   5  |                              |   6  |   7  |   8  |   9  |   0  |   sz   |
 * |--------+------+------+------+------+------|                              |------+------+------+------+------+--------|
 * |  Tab   |   Q  |   W  |   E  |   R  |   T  |                              |   Z  |   U  |   I  |   O  |   P  |   ue   |
 * |--------+------+------+------+------+------|                              |------+------+------+------+------+--------|
 * |  Bksp  |GUI/A |ALT/S |SFT/D |CTL/F |   G  |                              |   H  |CTL/J |SFT/K |ALT/L |GUI/oe|   ae   |
 * |--------+------+------+------+------+------+-------------.  ,-------------+------+------+------+------+------+--------|
 * |  ____  |   Y  |   X  |   C  |   V  |   B  | F12TD|SFTF12|  |      |      |   N  |   M  |   ,  |   .  |   -  |  ____  |
 * `----------------------+------+------+------+------+------|  |------+------+------+------+------+----------------------'
 *                        |      | ALT+ |  SYM | Enter|      |  | Del  | Space|  NAV |  MEH | HYPER|
 *                        |      | INS  |      |      |      |  |      |      |      |      |      |
 *                        `----------------------------------'  `----------------------------------'
 * ,-----------------------------------.                                              ,-----------------------------------.
 * | MUTE | ____ | _____ | ____ | ____ |                                              | MUTE | ____ | _____ | ____ | ____ |
 * `-----------------------------------'                                              `-----------------------------------'
 */
    [_WIN] = LAYOUT_elora_hlc(
     KC_ESC  , KC_1  ,  KC_2   ,  KC_3  ,   KC_4 ,   KC_5 ,                                        KC_6 ,  KC_7 ,  KC_8 ,   KC_9 ,  KC_0 , KC_MINS,
     KC_TAB  , KC_Q  ,  KC_W   ,  KC_E  ,   KC_R ,   KC_T ,                                        KC_Y ,  KC_U ,  KC_I ,   KC_O ,  KC_P , KC_LBRC,
     KC_BSPC , LGUI_A,  LGALT_S,  LSFT_D,  LCTL_F,   KC_G ,                                        KC_H , RCTL_J, RSFT_K,  RALT_L,RGUI_OE, KC_QUOT,
     KC_NO   , KC_Z  ,  KC_X   ,  KC_C  ,   KC_V ,   KC_B , TD_F12S,SFT_F12,     KC_NO  , KC_NO  , KC_N ,  KC_M ,KC_COMM, KC_DOT ,KC_SLSH, KC_NO,
                                  KC_NO , ALT_INS,    SYM , KC_ENT , KC_NO ,     KC_DEL , KC_SPC , NAV  , KC_MEH,KC_HYPR,
     KC_MUTE, KC_NO,  KC_NO, KC_NO, KC_NO,                                                                KC_MUTE, KC_NO, KC_NO, KC_NO, KC_NO
    ),

/*
 * Base Layer: MAC
 *
 * ,-------------------------------------------.                              ,-------------------------------------------.
 * |  Esc   |   1  |   2  |   3  |   4  |   5  |                              |   6  |   7  |   8  |   9  |   0  |   sz   |
 * |--------+------+------+------+------+------|                              |------+------+------+------+------+--------|
 * |  Tab   |   Q  |   W  |   E  |   R  |   T  |                              |   Z  |   U  |   I  |   O  |   P  |   ue   |
 * |--------+------+------+------+------+------|                              |------+------+------+------+------+--------|
 * |  Bksp  |CTL/A |ALT/S |SFT/D |GUI/F |   G  |                              |   H  |GUI/J |SFT/K |ALT/L |CTL/oe|   ae   |
 * |--------+------+------+------+------+------+-------------.  ,-------------+------+------+------+------+------+--------|
 * |  ____  |   Y  |   X  |   C  |   V  |   B  | F12TD|SFTF12|  |      |      |   N  |   M  |   ,  |   .  |   -  |  ____  |
 * `----------------------+------+------+------+------+------|  |------+------+------+------+------+----------------------'
 *                        |      | ALT+ |SYMMAC| Enter|      |  | Del  | Space|  NAV |  MEH | HYPER|
 *                        |      | INS  |      |      |      |  |      |      |      |      |      |
 *                        `----------------------------------'  `----------------------------------'
 * ,-----------------------------------.                                              ,-----------------------------------.
 * | MUTE | ____ | _____ | ____ | ____ |                                              | MUTE | ____ | _____ | ____ | ____ |
 * `-----------------------------------'                                              `-----------------------------------'
 */
    [_MAC] = LAYOUT_elora_hlc(
     KC_ESC  , KC_1  ,  KC_2   ,  KC_3  ,   KC_4 ,   KC_5 ,                                        KC_6 ,  KC_7 ,  KC_8 ,   KC_9 ,  KC_0 , KC_MINS,
     KC_TAB  , KC_Q  ,  KC_W   ,  KC_E  ,   KC_R ,   KC_T ,                                        KC_Y ,  KC_U ,  KC_I ,   KC_O ,  KC_P , KC_LBRC,
     KC_BSPC , LCTL_A,  LGALT_S,  LSFT_D,  LGUI_F,   KC_G ,                                        KC_H , RGUI_J, RSFT_K,  RALT_L,RCTL_OE, KC_QUOT,
     KC_NO   , KC_Z  ,  KC_X   ,  KC_C  ,   KC_V ,   KC_B ,  TD_F12S , SFT_F12,      KC_NO  , KC_NO  , KC_N ,  KC_M ,KC_COMM, KC_DOT ,KC_SLSH, KC_NO,
                                  KC_NO , ALT_INS, SYMMAC , KC_ENT , KC_NO ,      KC_DEL , KC_SPC , NAVMAC, KC_MEH,KC_HYPR,
     KC_MUTE, KC_NO,  KC_NO, KC_NO, KC_NO,                                                                KC_MUTE, KC_NO, KC_NO, KC_NO, KC_NO
    ),

/*
 * Nav Layer: Media, navigation
 *
 * ,-------------------------------------------.                              ,-------------------------------------------.
 * |  Reset |      |      |      |      | VolUp|                              |      |      |      |      |      |        |
 * |--------+------+------+------+------+------|                              |------+------+------+------+------+--------|
 * |        |      | Home |Insert| End  | VolDn|                              | PgUp | Home |   ↑  | End  |      | Delete |
 * |--------+------+------+------+------+------|                              |------+------+------+------+------+--------|
 * |        |  GUI |  ALT |  SFT |  CTL |VolMut|                              | PgDn |  ←   |   ↓  |   →  |      |        |
 * |--------+------+------+------+------+------+-------------.  ,-------------+------+------+------+------+------+--------|
 * |        |      |      |      |      |      |      |ScLck |  |M Play|      | Pause|      |Insert|      |      | PrtSc  |
 * `----------------------+------+------+------+------+------|  |------+------+------+------+------+----------------------'
 *                        |  WIN |  MAC |      |      |      |  |M Prev|M Next|      |      |      |
 *                        |      |      |      |      |      |  |      |      |      |      |      |
 *                        `----------------------------------'  `----------------------------------'
 * ,-----------------------------------.                                              ,-----------------------------------.
 * |      |      |       |      |      |                                              |      |      |       |      |      |
 * `-----------------------------------'                                              `-----------------------------------'
 */
    [_NAV] = LAYOUT_elora_hlc(
      QK_BOOT, _______, _______, _______, _______, KC_VOLU,                                     _______, _______, _______, _______, _______, _______,
      _______, _______, KC_HOME, KC_INS , KC_END , KC_VOLD,                                     KC_PGUP, KC_HOME, KC_UP,   KC_END,  _______, KC_DEL ,
      _______, KC_LGUI, KC_LALT, KC_LSFT, KC_LCTL, KC_MUTE,                                     KC_PGDN, KC_LEFT, KC_DOWN, KC_RGHT, _______, _______,
       ADJUST, _______, _______, _______, _______, _______, _______, KC_SCRL, KC_MPLY, _______,KC_PAUSE, _______, KC_INS , _______, _______, KC_PSCR,
                                    WIN ,    MAC , _______, _______, _______, KC_MPRV, KC_MNXT, _______, _______, _______,
     _______, _______,  _______, _______, _______,                                                       _______, _______, _______, _______, _______
    ),

/*
 * Sym Layer: Numbers and symbols
 *
 * ,-------------------------------------------.                              ,-------------------------------------------.
 * |    ESC |  F1  |  F2  |  F3  |  F4  |  F5  |                              |   F6 |  F7  |  F8  |  F9  |  F10 |   F11  |
 * |--------+------+------+------+------+------|                              |------+------+------+------+------+--------|
 * |        |  °   |  _   |  [   |  ]   |  ^   |                              |   !  |  <   |  >   |  =   |  &   |   F12  |
 * |--------+------+------+------+------+------|                              |------+------+------+------+------+--------|
 * |        |  \   |  /   |  {   |  }   |  *   |                              |   ?  |  (   |  )   |  -   |  :   |        |
 * |--------+------+------+------+------+------+-------------.  ,-------------+------+------+------+------+------+--------|
 * |        |  #   |  $   |  |   |  ~   |  `   |CPSLCK|      |  |      |      |   +  |  %   |  "   |  '   |  ;   |        |
 * `----------------------+------+------+------+------+------|  |------+------+------+------+------+----------------------'
 *                        |      |      |      |      |      |  |      |      |      |      |      |
 *                        |      |      |      |      |      |  |      |      |      |      |      |
 *                        `----------------------------------'  `----------------------------------'
 * ,-----------------------------------.                                              ,-----------------------------------.
 * |      |      |       |      |      |                                              |      |      |       |      |      |
 * `-----------------------------------'                                              `-----------------------------------'
 */
    [_SYM] = LAYOUT_elora_hlc(
      KC_ESC,   KC_F1,   KC_F2,   KC_F3,   KC_F4,   KC_F5,                                       KC_F6,   KC_F7,   KC_F8,   KC_F9,  KC_F10, KC_F11 ,
     _______, SYM_DEG,SYM_USCR,SYM_LSQB,SYM_RSQB,SYM_DACH,                                    SYM_EXCL,SYM_LPOB,SYM_RPOB, SYM_EQL, SYM_AMP, KC_F12 ,
     _______,SYM_BSLS,SYM_SLSH,SYM_LCUB,SYM_RCUB,SYM_STAR,                                    SYM_QSTN,SYM_LBRK,SYM_RBRK,SYM_MINS,SYM_COLN, _______,
     _______,SYM_HASH,SYM_DOLR,SYM_PIPE,SYM_TLDE,SYM_BTCK, KC_CAPS, _______, _______, _______,SYM_PLUS,SYM_PERC,SYM_DQOT,SYM_SQOT,SYM_SCLN, _______,
                                 _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,
     _______, _______,  _______, _______, _______,                                                       _______, _______, _______, _______, _______
    ),

/*
 * Mac Sym Layer: Numbers and symbols for Mac
 *
 * ,-------------------------------------------.                              ,-------------------------------------------.
 * |    ESC |  F1  |  F2  |  F3  |  F4  |  F5  |                              |   F6 |  F7  |  F8  |  F9  |  F10 |   F11  |
 * |--------+------+------+------+------+------|                              |------+------+------+------+------+--------|
 * |        |  °   |  _   |  [   |  ]   |  ^   |                              |   !  |  <   |  >   |  =   |  &   |   F12  |
 * |--------+------+------+------+------+------|                              |------+------+------+------+------+--------|
 * |        |  \   |  /   |  {   |  }   |  *   |                              |   ?  |  (   |  )   |  -   |  :   |        |
 * |--------+------+------+------+------+------+-------------.  ,-------------+------+------+------+------+------+--------|
 * |        |  #   |  $   |  |   |  ~   |  `   |CPSLCK|      |  |      |      |   +  |  %   |  "   |  '   |  ;   |        |
 * `----------------------+------+------+------+------+------|  |------+------+------+------+------+----------------------'
 *                        |      |      |      |      |      |  |      |      |      |      |      |
 *                        |      |      |      |      |      |  |      |      |      |      |      |
 *                        `----------------------------------'  `----------------------------------'
 * ,-----------------------------------.                                              ,-----------------------------------.
 * |      |      |       |      |      |                                              |      |      |       |      |      |
 * `-----------------------------------'                                              `-----------------------------------'
 */
    [_SYMMAC] = LAYOUT_elora_hlc(
        KC_ESC,   KC_F1,   KC_F2,   KC_F3,   KC_F4,   KC_F5,                                       KC_F6,   KC_F7,   KC_F8,   KC_F9,  KC_F10, KC_F11 ,
       _______,SYM_RPOB,SYM_USCR,SYM_LSQB,SYM_RSQB,SYM_LPOB,                                    SYM_EXCL,SYM_DACH,SYM_DEG, SYM_EQL, SYM_AMP, KC_F12 ,
       _______,SYM_BSLS,SYM_SLSH,SYM_LCUB,SYM_RCUB,SYM_STAR,                                    SYM_QSTN,SYM_LBRK,SYM_RBRK,SYM_MINS,SYM_COLN, _______,
       _______,SYM_HASH,SYM_DOLR,SYM_MPIP,SYM_TLDE,SYM_BTCK, KC_CAPS, _______, _______, _______,SYM_PLUS,SYM_PERC,SYM_DQOT,SYM_SQOT,SYM_SCLN, _______,
                                   _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,
       _______, _______,  _______, _______, _______,                                                       _______, _______, _______, _______, _______
      ),



/*
 * Adjust Layer: Default layer settings, RGB
 *
 * ,-------------------------------------------.                              ,-------------------------------------------.
 * |        |      |      |      |      |      |                              |      |      |      |      |      |        |
 * |--------+------+------+------+------+------|                              |------+------+------+------+------+--------|
 * |        |      |      |  WIN |      |      |                              |      |      |      |      |      |        |
 * |--------+------+------+------+------+------|                              |------+------+------+------+------+--------|
 * |        |      |      |  MAC |      |      |                              | TOG  | SAI  | HUI  | VAI  | MOD  |        |
 * |--------+------+------+------+------+------+-------------.  ,-------------+------+------+------+------+------+--------|
 * |        |      |      |Colmak|      |      |      |      |  |      |      |      | SAD  | HUD  | VAD  | RMOD |        |
 * `----------------------+------+------+------+------+------|  |------+------+------+------+------+----------------------'
 *                        |      |      |      |      |      |  |      |      |      |      |      |
 *                        |      |      |      |      |      |  |      |      |      |      |      |
 *                        `----------------------------------'  `----------------------------------'
 * ,-----------------------------------.                                              ,-----------------------------------.
 * |      |      |       |      |      |                                              |      |      |       |      |      |
 * `-----------------------------------'                                              `-----------------------------------'
 */
    [_ADJUST] = LAYOUT_elora_hlc(
      _______, _______, _______, _______, _______, _______,                                    _______, _______, _______, _______,  _______, _______,
      _______, _______, _______, _______, _______, _______,                                    _______, _______, _______, _______,  _______, _______,
      _______, UG_NEXT, UG_VALU, UG_HUEU, UG_SATU, UG_TOGG,                                    RM_TOGG, RM_SATU, RM_HUEU, RM_VALU,  RM_NEXT, _______,
      _______, UG_PREV, UG_VALD, UG_HUED, UG_SATD, _______,_______, _______, _______, _______, _______, RM_SATD, RM_HUED, RM_VALD, RM_PREV, _______,
                                 _______, _______, _______,_______, _______, _______, _______, _______, _______, _______,
     _______, _______,  _______, _______, _______,                                                       _______, _______, _______, _______, _______
    ),

/*
 * Nav Layer Mac: Media, navigation
 *
 * ,-------------------------------------------.                              ,-------------------------------------------.
 * |  Reset |      |      |      |      | VolUp|                              |      |      |      |      |      |        |
 * |--------+------+------+------+------+------|                              |------+------+------+------+------+--------|
 * |        |      | Home |Insert| End  | VolDn|                              | PgUp | Home |   ↑  | End  |      | Delete |
 * |--------+------+------+------+------+------|                              |------+------+------+------+------+--------|
 * |        |  GUI |  ALT |  SFT |  CTL |VolMut|                              | PgDn |  ←   |   ↓  |   →  |      |        |
 * |--------+------+------+------+------+------+-------------.  ,-------------+------+------+------+------+------+--------|
 * |        |      |      |      |      |      |      |ScLck |  |M Play|      | Pause|      |Insert|      |      | PrtSc  |
 * `----------------------+------+------+------+------+------|  |------+------+------+------+------+----------------------'
 *                        |  WIN |  MAC |      |      |      |  |M Prev|M Next|      |      |      |
 *                        |      |      |      |      |      |  |      |      |      |      |      |
 *                        `----------------------------------'  `----------------------------------'
 * ,-----------------------------------.                                              ,-----------------------------------.
 * |      |      |       |      |      |                                              |      |      |       |      |      |
 * `-----------------------------------'                                              `-----------------------------------'
 */
    [_NAVMAC] = LAYOUT_elora_hlc(
      QK_BOOT, _______, _______, _______, _______, KC_VOLU,                                     _______, _______, _______, _______, _______, _______,
      _______, _______, KC_HOME, KC_INS , KC_END , KC_VOLD,                                     KC_PGUP, KC_HOME, KC_UP,   KC_END,  _______, KC_DEL ,
      _______, KC_LCTL, KC_LALT, KC_LSFT, KC_LGUI, KC_MUTE,                                     KC_PGDN, KC_LEFT, KC_DOWN, KC_RGHT, _______, _______,
      _______, _______, _______, _______, _______, _______, _______, KC_SCRL, KC_MPLY, _______,KC_PAUSE, _______, KC_INS , _______, _______, KC_PSCR,
                                    WIN ,    MAC , _______, _______, _______, KC_MPRV, KC_MNXT, _______, _______, _______,
     _______, _______,  _______, _______, _______,                                                       _______, _______, _______, _______, _______
    ),
// /*
//  * Layer template
//  *
//  * ,-------------------------------------------.                              ,-------------------------------------------.
//  * |        |      |      |      |      |      |                              |      |      |      |      |      |        |
//  * |--------+------+------+------+------+------|                              |------+------+------+------+------+--------|
//  * |        |      |      |      |      |      |                              |      |      |      |      |      |        |
//  * |--------+------+------+------+------+------|                              |------+------+------+------+------+--------|
//  * |        |      |      |      |      |      |                              |      |      |      |      |      |        |
//  * |--------+------+------+------+------+------+-------------.  ,-------------+------+------+------+------+------+--------|
//  * |        |      |      |      |      |      |      |      |  |      |      |      |      |      |      |      |        |
//  * `----------------------+------+------+------+------+------|  |------+------+------+------+------+----------------------'
//  *                        |      |      |      |      |      |  |      |      |      |      |      |
//  *                        |      |      |      |      |      |  |      |      |      |      |      |
//  *                        `----------------------------------'  `----------------------------------'
//  * ,-----------------------------------.                                              ,-----------------------------------.
//  * |      |      |       |      |      |                                              |      |      |       |      |      |
//  * `-----------------------------------'                                              `-----------------------------------'
//  */
//     [_LAYERINDEX] = LAYOUT_elora_hlc(
//       _______, _______, _______, _______, _______, _______,                                     _______, _______, _______, _______, _______, _______,
//       _______, _______, _______, _______, _______, _______,                                     _______, _______, _______, _______, _______, _______,
//       _______, _______, _______, _______, _______, _______,                                     _______, _______, _______, _______, _______, _______,
//       _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,
//                                  _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,
//      _______, _______,  _______, _______, _______,                                                       _______, _______, _______, _______, _______
//     ),
//
};

layer_state_t layer_state_set_user(layer_state_t state) {
    isMac = IS_LAYER_ON_STATE(state, _MAC);
    return state;
}

// increase the tapping terms for the pinkys slightly,
// those fingers are weak AF
uint16_t get_tapping_term(uint16_t keycode, keyrecord_t *record) {
    if (isMac) {
        switch (keycode) {
            case SFT_T(KC_LCTL):
                return TAPPING_TERM + 10;
            case SFT_T(KC_RCTL):
                return TAPPING_TERM + 10;
            default:
                return TAPPING_TERM;
        }
    } else {
        switch (keycode) {
            case SFT_T(KC_LGUI):
                return TAPPING_TERM + 10;
            case SFT_T(KC_RGUI):
                return TAPPING_TERM + 10;
            default:
                return TAPPING_TERM;
        }
    }
}

#ifdef ENCODER_ENABLE
bool encoder_update_user(uint8_t index, bool clockwise) {
    if (index == 3) {
      if (clockwise) {
          tap_code(KC_VOLU);
      } else {
          tap_code(KC_VOLD);
      }
    }
    return false;
}
#endif
